...
{
    if( udrv_flash_write(MCU_CERT_CONFIG_NVM_ADDR + offset, (uint32_t)size, (uint8_t *)src) == UDRV_RETURN_OK )
    {
        return size;
    }
    return 0;
...
{
    if( udrv_flash_read(MCU_CERT_CONFIG_NVM_ADDR + offset, (uint32_t)size, (uint8_t *)dest) == UDRV_RETURN_OK)
    {
        return size;
    }
    return 0;
...
bool NvmmReset( uint16_t size, uint16_t offset )
{
    ...
    if( udrv_flash_erase(MCU_CERT_CONFIG_NVM_ADDR + offset,size) == UDRV_RETURN_OK )
    {
        return true;
    }
    return false;

...
/*
 *  RAK Sub_region AS923 - 1 -2 -3 -4
 *  The offset of the channel has been calculated       
 */
#ifdef AS923_SUB_REGION    
ChannelParams_t AS923_Ch1[] ={
    { 923200000, 0, { ( ( DR_5 << 4 ) | DR_0 ) }, 0 },  // AS923 (Default is AS923-1)
    { 923200000, 0, { ( ( DR_5 << 4 ) | DR_0 ) }, 0 },  // AS923-1
    { 921400000, 0, { ( ( DR_5 << 4 ) | DR_0 ) }, 0 },  // AS923-2
    { 916600000, 0, { ( ( DR_5 << 4 ) | DR_0 ) }, 0 },  // AS923-3
    { 917300000, 0, { ( ( DR_5 << 4 ) | DR_0 ) }, 0 },  // AS923-4
};

ChannelParams_t AS923_Ch2[] ={
    { 923400000, 0, { ( ( DR_5 << 4 ) | DR_0 ) }, 0 },  // AS923 (Defalut is AS923-1)
    { 923400000, 0, { ( ( DR_5 << 4 ) | DR_0 ) }, 0 },  // AS923-1
    { 921600000, 0, { ( ( DR_5 << 4 ) | DR_0 ) }, 0 },  // AS923-2
    { 916800000, 0, { ( ( DR_5 << 4 ) | DR_0 ) }, 0 },  // AS923-3
    { 917500000, 0, { ( ( DR_5 << 4 ) | DR_0 ) }, 0 },  // AS923-4
};
...
{
    /* Exceeding legal frequency band */
    if (band > 4)
        return ;
...
bool RegionAS923TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir )
{
    ...
    LORA_TEST_DEBUG("TX Frequency %d",RegionNvmGroup2->Channels[txConfig->Channel].Frequency);
...
LoRaMacStatus_t RegionAS923NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, TimerTime_t* time, TimerTime_t* aggregatedTimeOff )
{
    ...
    if( status == LORAMAC_STATUS_OK )
    {
        ...
        uint32_t lbt_time = service_lora_get_lbt_scantime();
        for( uint8_t  i = 0, j = randr( 0, nbEnabledChannels - 1 ); i < AS923_MAX_NB_CHANNELS; i++ )
        {
            ...
            if( Radio.IsChannelFree( RegionNvmGroup2->Channels[channelNext].Frequency, 200000, lbt_rssi, lbt_time ) == true )
            {
                // Free channel found
                *channel = channelNext;

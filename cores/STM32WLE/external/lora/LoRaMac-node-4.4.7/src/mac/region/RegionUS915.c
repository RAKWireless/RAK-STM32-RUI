...
{
    if(US915_SingleChannel.IsSingleChannel != NULL)
    {
    ...
    RegionCommonChanMaskCopy(RegionNvmGroup2->ChannelsDefaultMask,RegionNvmGroup2->ChannelsMask,CHANNELS_MASK_SIZE);
}
...
bool RegionUS915ChanMaskSet( ChanMaskSetParams_t* chanMaskSet )
{
    ...
    //     ( nbChannels > 0 ) )
    // {
    //     udrv_serial_printf((SERIAL_PORT)ATCMD_IO_SERIAL_PORT, "(%s) (%s) (%d)\r\n",__FILE__,__func__, __LINE__);
    //     return false;
    // }
...
bool RegionUS915TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir )
{
    ...
    *txPower = txPowerLimited;
    LORA_TEST_DEBUG("SetChannel %d",txConfig->Channel);
    LORA_TEST_DEBUG("channel mask %04X %04X %04X %04X %04X",RegionNvmGroup2->ChannelsMask[0],
    RegionNvmGroup2->ChannelsMask[1],RegionNvmGroup2->ChannelsMask[2],RegionNvmGroup2->ChannelsMask[3],
    RegionNvmGroup2->ChannelsMask[4]);

    LORA_TEST_DEBUG("default mask %04X %04X %04X %04X %04X",RegionNvmGroup2->ChannelsDefaultMask[0],
    RegionNvmGroup2->ChannelsDefaultMask[1],RegionNvmGroup2->ChannelsDefaultMask[2],RegionNvmGroup2->ChannelsDefaultMask[3],
    RegionNvmGroup2->ChannelsDefaultMask[4]);

    LORA_TEST_DEBUG("remain mask %04X %04X %04X %04X %04X",RegionNvmGroup1->ChannelsMaskRemaining[0],
    RegionNvmGroup1->ChannelsMaskRemaining[1],RegionNvmGroup1->ChannelsMaskRemaining[2],RegionNvmGroup1->ChannelsMaskRemaining[3],
    RegionNvmGroup1->ChannelsMaskRemaining[4]);

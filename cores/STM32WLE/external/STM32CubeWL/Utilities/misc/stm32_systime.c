...
# 46 "/home/eddieho/tmp/phase2_release/rui-v3/external/STM32CubeWL/Utilities/misc/stm32_systime.c" 2
...
# 58 "/home/eddieho/tmp/phase2_release/rui-v3/external/STM32CubeWL/Utilities/misc/stm32_systime.h"
...
# 59 "/home/eddieho/tmp/phase2_release/rui-v3/external/STM32CubeWL/Utilities/misc/stm32_systime.h" 2
...
# 47 "/home/eddieho/tmp/phase2_release/rui-v3/external/STM32CubeWL/Utilities/misc/stm32_systime.c" 2
# 62 "/home/eddieho/tmp/phase2_release/rui-v3/external/STM32CubeWL/Utilities/misc/stm32_systime.c"
#define END_OF_FEBRUARY_LEAP 60





#define END_OF_JULY_LEAP 213





#define END_OF_FEBRUARY_NORM 59





#define END_OF_JULY_NORM 212
...
# 175 "/home/eddieho/tmp/phase2_release/rui-v3/external/STM32CubeWL/Utilities/misc/stm32_systime.c"
const char *WeekDayString[]={ "Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat" };
# 186 "/home/eddieho/tmp/phase2_release/rui-v3/external/STM32CubeWL/Utilities/misc/stm32_systime.c"
...
# 200 "/home/eddieho/tmp/phase2_release/rui-v3/external/STM32CubeWL/Utilities/misc/stm32_systime.c"
SysTime_t SysTimeAdd( SysTime_t a, SysTime_t b )
{
...
uint32_t SysTimeMkTime( const struct tm* localtime )
{
  uint32_t nbdays;
  uint32_t nbsecs;
  uint32_t year = localtime->tm_year - 68;
  ...
  nbdays = ( ( ( ( ( ( uint32_t ) 365U ) * 3 + ( ( uint32_t ) 366U ) ) * year ) + ( 4 ) -1 ) / ( 4 ) );

  nbdays += ( ( ( ( ( localtime->tm_mon ) * ( 30 + 31 ) ) + 1 ) >> 1 ) -
        ( ( ( correctionMonth[year % 4] >> ( ( localtime->tm_mon ) * 2 ) ) & 0x03 ) ) );

  nbdays += ( localtime->tm_mday - 1 );


  nbsecs = nbdays * ( ( uint32_t )86400U );

  nbsecs += ( ( uint32_t )localtime->tm_sec +
        ( ( uint32_t )localtime->tm_min * ( ( uint32_t ) 60U ) ) +
        ( ( uint32_t )localtime->tm_hour * ( ( uint32_t ) 3600U ) ) );
  return nbsecs - ( ( ( ( uint32_t ) 366U ) + ( ( uint32_t ) 365U ) ) * ( ( uint32_t )86400U ) );
}
...
void SysTimeLocalTime( const uint32_t timestamp, struct tm *localtime )
{
  ...
  CalendarDiv86400( timestamp + ( ( ( ( uint32_t ) 366U ) + ( ( uint32_t ) 365U ) ) * ( ( uint32_t )86400U ) ), &days, &seconds );
  ...
  localtime->tm_year = ( ( ( days ) * 91867 + 22750 ) >> 25 );
  days-= ( ( ( ( ( ( uint32_t ) 365U ) * 3 + ( ( uint32_t ) 366U ) ) * localtime->tm_year ) + 3 ) >>2 );
  ...
  weekDays += ( ( ( ( localtime->tm_year * 5 ) ) + 3 ) >>2 );
  weekDays += days;
  localtime->tm_wday = ( ( weekDays ) -( ( ( ( ( weekDays ) + 1 ) * 299593 ) >> 21 ) * 7 ) );

  days -= ( ( ( ( ( localtime->tm_mon ) * ( 30 + 31 ) ) + 1 ) >> 1 ) -
        ( ( ( correctionMonth[localtime->tm_year % 4] >> ( ( localtime->tm_mon ) * 2 ) ) & 0x03 ) ) );


  localtime->tm_mday = days + 1;

  localtime->tm_year += 68;

  localtime->tm_isdst = -1;
}
# 382 "/home/eddieho/tmp/phase2_release/rui-v3/external/STM32CubeWL/Utilities/misc/stm32_systime.c"
static uint32_t CalendarGetMonth( uint32_t days, uint32_t year )
{
  uint32_t month;
  if( ( year % 4 ) == 0 )
  {
    ...
    {
      month = CalendarDiv61( ( days - 60 ) * 2 ) + 2;
    }
    else
    {
      month = CalendarDiv61( ( days - 213 ) * 2 ) + 7;
    }
  }
  else
  {
    ...
    {
      month = CalendarDiv61( ( days - 59 ) * 2 ) + 2;
    }
    else
    {
      month = CalendarDiv61( ( days - 212 ) * 2 ) + 7;
    }
  }
  return month;
}
...
static void CalendarDiv86400( uint32_t in, uint32_t* out, uint32_t* remainder )
{
  ...
  uint32_t outTemp = 0;
  uint32_t divResult = ( ( ( in ) >> 18 ) + ( ( in ) >> 17 ) );

  while( divResult >=1 )
  {
    outTemp += divResult;
    in -= divResult * 86400;
    divResult= ( ( ( in ) >> 18 ) + ( ( in ) >> 17 ) );
  }
  if( in >= 86400 )
  {
    outTemp += 1;
...
static uint32_t CalendarDiv61( uint32_t in )
{
  ...
  uint32_t outTemp = 0;
  uint32_t divResult = ( ( ( in ) * 68759 ) >> 22 );
  while( divResult >=1 )
  {
    outTemp += divResult;
    in -= divResult * 61;
    divResult = ( ( ( in ) * 68759 ) >> 22 );
  }
  if( in >= 61 )
  {
    outTemp += 1;
...
static void CalendarDiv60( uint32_t in, uint32_t* out, uint32_t* remainder )
{
  ...
  uint32_t outTemp = 0;
  uint32_t divResult = ( ( ( in ) * 17476 ) >> 20 );

  while( divResult >=1 )
  {
    outTemp += divResult;
    in -= divResult * 60;
    divResult = ( ( ( in ) * 17476 ) >> 20 );
  }
  if( in >= 60 )
  {
    outTemp += 1;
